...
class DSDatePicker {
  ...
  init() {
    ...
    this.calendarButtonElement.setAttribute(
      "data-button",
      `datepicker-${this.inputElement.id}-toggle`
    )
  ...
  openDialog() {
    ...
    if (this.isMultipleInput) {
      leftOffset = `${
        this.calendarButtonElement.offsetLeft + this.calendarButtonElement.offsetWidth + 16
      }px`
  ...
  setCurrentDate(focus = true) {
    ...
    const filteredDays = this.calendarDays.filter(
      calendarDay => calendarDay.button.classList.contains("fully-hidden") === false
    )
  ...
  setDate(date) {
    if (this.isMultipleInput) {
      ...
      this.inputElement.value = `${this.leadingZeroes(date.getDate())}/${this.leadingZeroes(
        date.getMonth() + 1
      )}/${date.getFullYear()}`

      switch (this.datePickerParent.dataset.dateformat) {
        case "YMD":
          this.inputElement.value = `${date.getFullYear()}/${this.leadingZeroes(
            date.getMonth() + 1
          )}/${this.leadingZeroes(date.getDate())}`
          break
        case "MDY":
          this.inputElement.value = `${this.leadingZeroes(
            date.getMonth() + 1
          )}/${this.leadingZeroes(date.getDate())}/${date.getFullYear()}`
          break
        case "DMY":
        default:
          this.inputElement.value = `${this.leadingZeroes(date.getDate())}/${this.leadingZeroes(
...
.ds_datepicker {
  ...
  &__dialog {
    ...
    &__table {
      tbody:focus-within {
        outline: 0.25rem solid var(--color-princeton-orange-on-white);
      ...
      button {
        ...
        &[aria-disabled="true"] {
          ...
          &:focus {
            ...
            box-shadow: inset 0 0 0 1px var(--color-princeton-orange-on-white),
              0 0 0 1px var(--color-princeton-orange-on-white);
            outline: 0;
          }
        }
        ...
        &.ds_datepicker__current {
          ...
          outline: 3px solid var(--color-princeton-orange-on-white) !important;
          outline-offset: -3px;
        }
